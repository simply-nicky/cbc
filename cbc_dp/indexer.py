...
import pygmo
from . import utils
from .feat_detect import FrameSetup
from .model import RectModel

class IndexSetup(FrameSetup):
    """
    Indexer setup class
    ...
    """
    def __init__(self, det_pos, num_ap, pix_size):
        super(IndexSetup, self).__init__(pix_size, det_pos)
        self.num_ap = num_ap

    def kout_exp(self, lines, vec):
        """
        Generate the experimentally measured deiffraction streaks outcoming wavevectors
        """
        return utils.kout(lines * self.pix_size, self.det_pos, vec)
    ...
    def det_pts(self, kout, vec):
        ...
        det_x = self.det_pos[2] * (1 + vec[2]) * np.tan(theta) * np.cos(phi)
        det_y = self.det_pos[2] * (1 + vec[2]) * np.tan(theta) * np.sin(phi)
        return (np.stack((det_x, det_y), axis=-1) + self.det_pos[:2] * (1 + vec[:2])) / self.pix_size
...
class AbcCBI(metaclass=ABCMeta):
    ...
    def __init__(self, lines, setup, rec_basis, tol, pen_coeff):
        self.lines, self.setup, self.pen_coeff = lines, setup, pen_coeff
...
class FCBI(AbcCBI):
    ...
    pen_coeff                       - fitness penalty coefficient
    """
    def __init__(self, lines, setup, rec_basis, tol=(0.1, 0.05), pen_coeff=10):
        super(FCBI, self).__init__(lines, setup, rec_basis.ravel(), tol, pen_coeff)
...
class RCBI(AbcCBI):
    ...
    pen_coeff                           - fitness penalty coefficient
    """
    def __init__(self, lines, setup, rec_basis, tol=(0.05, 0.1, np.radians(5)), pen_coeff=10):
        super(RCBI, self).__init__(lines, setup, rec_basis, tol, pen_coeff)
